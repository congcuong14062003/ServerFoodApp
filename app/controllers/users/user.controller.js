...
export const userSignup = async (req, res) => {
    try {
        const { phone_number, name, address, password } = req.body;
        
        if (!phone_number || !name || !address || !password) {
            return res.status(400).json({ success: false, message: "Please fill all information" });
        }

        const checkPhoneNumber = await Users.checkPhoneNumber(phone_number);
        if (checkPhoneNumber) {
